		...
		init: function () {
			...
			var oPage = new Page({
				...
				content: [
					new VBox({
						items: [
							new Text({ text: "Download documents using the token from your email", wrapping: true }),
							new VBox({
								items: [
									new Text({ text: "Enter Download Token", fontWeight: "Bold" }),
									new Text({ text: "Paste the token you received via email from the supplier" }),
									new Label({ text: "Download Token: *" }),
									oTokenInput,
									new HBox({ items: [oRetrieveBtn], justifyContent: "Start" }),
									oRetrieveResult
								],
								width: "100%",
								spacing: "0.5rem"
							...
							new VBox({
								items: [
									new Text({ text: "📄 Document Details", visible: false, id: "docTitle" }),
									oDocumentInfo,
									new HBox({ items: [oDownloadBtn, oConfirmBtn], justifyContent: "Start", spacing: "1rem" }),
			...
			async function downloadDocument() {
				...
				try {
					...
					a.href = url;
					a.download = (currentShipment && currentShipment.documentName) || "document";
